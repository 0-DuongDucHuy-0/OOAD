...
Product.getOne = (id, callback) => {
    ketnoi.query("SELECT * FROM auctions JOIN books ON auctions.BookID = books.BookID JOIN bids ON auctions.AuctionID = bids.AuctionID WHERE bids.AuctionID = ? ORDER BY bids.BidAmount DESC;", [id], (err, data) => {
        if (err) {
            callback(err, null);
        } else {
